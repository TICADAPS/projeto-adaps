...
$controle = $modelControle->find("idMedico=:id", "id=$idMedico");
$idCalendario = $controle->idCalendario;
$calendario = $modelCalendario->find("idCalendario=:id", "id=$idCalendario");
...
<div class="container">
    <h2 class="text-center p-3 bg-primary text-light">Período original</h2>
    <table class="border border-primary table table-hover table-striped table-responsive-sm">
        ...
        <tbody>
            ...
            <tr>
                <td><?= vemdata($calendario->PeriodoInicial) ?></td>
                <td><?= vemdata($calendario->PeriodoFinal) ?></td>
            </tr>

        </tbody>
    </table>
    <h2 class="text-center p-3 bg-primary text-light">Escolha um novo período</h2>
    <form action="Controller/tratarDados.php" method="post">
