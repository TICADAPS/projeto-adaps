        ...
        if ($user == "011.792.350-88") {
            ...
            if ($this->Crud->rowCount() == 1) {
                while ($ln = $this->Crud->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $_SESSION['chamada'] = $ln['Convocacao'];
                    $id = $_SESSION['idMed'];
                    $idcargo = $_SESSION['idCargo'];
                    //echo "<script>alert($idcargo)</script>";

                    if ($idcargo == 1) {
                        $sql = "SELECT * FROM medico_bolsista WHERE idMedico = :id LIMIT 1";
                        $this->Crud = $this->conectaDB()->prepare($sql);
                        ...
                        if ($this->Crud->rowCount() == 1) {
                            ...
                            $_SESSION['loginErro'] = "<b>O CPF informado não encontra-se na base de cadastro reserva.</b>";
                            header("Location:../login.php");
                            exit();
                        }
                    } else {
                        $sql2 = "SELECT * FROM tutor_municipio WHERE idTutor = :id LIMIT 1";
                        $this->Crud = $this->conectaDB()->prepare($sql2);
                        $this->Crud->bindValue(":id", $id);
                        $this->Crud->execute();
                        if ($this->Crud->rowCount() == 1) {
                            header("Location:../CalTutor.php"); exit();
                        } else {
                            $_SESSION['loginErro'] = "<b>O CPF informado não encontra-se na base de cadastro reserva.</b>";
                            header("Location:../login.php");
                            exit();
