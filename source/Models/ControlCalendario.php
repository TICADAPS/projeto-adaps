...
//        $this->FlagCienciaPortaria = $FlagCienciaPortaria;
    ...
    public function joinsBolsista(int $idBolsista): ?array {
        $joins = $this->read("select med.NomeMedico, vt.munic_origem,vt.munic_escolhido, ct.PeriodoInicial, ct.PeriodoFinal,
    ...
    public function remanejaBolsista(): ?array {
        $joins = $this->read("select vt.idMedico, m.NomeMedico, vt.munic_origem, 
        vt.munic_escolhido from vaga_tutoria vt inner join medico m on vt.idMedico = m.idMedico
        where vt.idMedico not in (select idMedico from ControleCalendario);");
        if ($this->fail() || !$joins->rowCount()) {
            $this->message = "Usuário não encontrado para o id informado";
            return null;
        }
    ...
    public function joinsEmailTutor(int $idBolsista): ?array {
        $joins = $this->read("select medico.NomeMedico, medico.email,medico.fone_zap,
